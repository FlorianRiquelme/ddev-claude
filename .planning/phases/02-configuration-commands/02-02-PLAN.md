...
<task type="auto">
  ...
  <action>
Create the commands directory structure: `commands/host/`

Create `commands/host/claude`:
```bash
#!/bin/bash
...
Key points:
- Command placed in commands/host/ since it uses ddev exec (runs on host, executes in container)
- Uses `ddev exec -s claude` to run in claude container (not web)
- --no-firewall flushes iptables rules temporarily
- Uses "$@" for proper argument passing (not $* which breaks on spaces)
- The `-- "$@"` pattern properly passes arguments through bash -c

Make executable: chmod +x
  </action>
  <verify>
Run: `bash -n commands/host/claude` (no syntax errors)
Run: `test -x commands/host/claude && echo "executable"` (executable)
Run: `head -10 commands/host/claude | grep -q "Description:" && echo "has DDEV metadata"`
  </verify>
  <done>
...
<verification>
1. commands/host/claude exists with proper DDEV metadata
2. commands/host/claude is executable
...
6. commands/host/claude uses "$@" for proper argument passing
</verification>
